...
- **`VideoView`** — just the video-painting surface, no chrome. Use this when you're building your
  ...
  directly — see [`src/MediaPlayer/Player.cs`](../MediaPlayer/Player.cs) in this repo for a
...
[`src/MediaPlayer`](../MediaPlayer) does. Only `Program.cs` (which backend package you reference,
...
[`src/MediaPlayer/Player.cs`](../MediaPlayer/Player.cs) in this repo is a complete one (it
...
var player = new LibMPlayerWinform.WinFormMPlayerControl { VideoPath = "...", MPlayerPath = "..." };
...
This is exactly what [`src/MediaPlayer`](../MediaPlayer) in this repo already does — see
